...
namespace GuildWar
{
    public partial class FrmMain
    {
        ...
        private void LoadParmas()
        {
            ...
            gameServerList.Add(GameServers.GuildWarsServer);
            ...
            this.txtRepeatCount.Text = SetConfig.Instance.ErrorRepeatCount.ToString();

            this.txtLower.Text = SetConfig.Instance.RangeLower.ToString();
            this.txtUpper.Text = SetConfig.Instance.RangeUpper.ToString();

            this.txtDialCount.Text = SetConfig.Instance.RedialCount.ToString();
        ...
        private DetectionParamsItem GetDetectionParamsItem()
        {
            ...
            if (TextHelper.IsNumber(this.txtRepeatCount.Text.Trim()))
            {
                ...
                if (count >= 0)
                {
                    ...
                    SetConfig.Instance.ErrorRepeatCount = count;
                }
                else
                {
            ...
            if (TextHelper.IsNumber(this.txtCaptchCount.Text.Trim()))
            {
                ...
                if (count > 0)
                {
                    ...
                    SetConfig.Instance.CaptchCount = count;
                }
                else
                {
            ...
            if (TextHelper.IsNumber(this.txtDialCount.Text.Trim()))
            {
                int count = TextHelper.StringToInt(this.txtDialCount.Text.Trim());
                if (count > 0)
                {
                    detectionItem.RedialCount = count;
                    SetConfig.Instance.RedialCount = count;
                }
                else
                {
                    sb.AppendLine(string.Format("{0}、请录入有效的扫号重启数，默认是3！", i++));
                }
            }
            else
            {
        ...
        private bool GetCustomRange(DetectionParamsItem detectionItem)
        {
            ...
            if (sb.Length > 0)
            {
                MessageBox.Show(sb.ToString(), "（GuildWar）提醒");
                return false;
            }
