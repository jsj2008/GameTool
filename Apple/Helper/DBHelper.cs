    ...
    public class DBHelper
    {
        ...
        public static int GetHistoryCount(params LoginState[] accountStates)
        {
            lock (lockGetObj)
            {
                if (IsSqlConOpened)
                {
                    string sql = "select count(distinct Id)  as itemsCount from AppleResult";
        ...
        public static IList<int> GetQueriedIndex()
        {
            ...
            if (IsSqlConOpened)
            {
                string sql = "select QueriedIndex from QueriedApple";
                using (SQLiteCommand sqlCmd = new SQLiteCommand(sql, sqlCon))
                {
                    using (SQLiteDataReader reader = sqlCmd.ExecuteReader())
                    {
        ...
        public static bool IsQueried(int userIndex)
        {
            if (IsSqlConOpened)
            {
                string sql = string.Format("SELECT QueriedIndex from QueriedApple where QueriedIndex={0}", userIndex);
                using (SQLiteCommand sqlCmd = new SQLiteCommand(sql, sqlCon))
                {
                    using (SQLiteDataReader reader = sqlCmd.ExecuteReader())
                    {
        ...
        public static bool ClearTable(params AppleTableName[] tableNames)
        {
            if (IsSqlConOpened)
            {
                foreach (AppleTableName t in tableNames)
                {
                    ClearTable(t);
                }
                return true;
        ...
        public static bool ClearTable(AppleTableName tableName)
        {
            if (IsSqlConOpened)
            {
        ...
        private static bool RunSql(string sql)
        {
            if (!string.IsNullOrEmpty(sql))
            {
                ...
                {
                    AppleLogManager.Instance.Error(string.Format("执行Insert sql时出错：{0}", ex.Message));
                }
            }
            return false;
        }
        ...
        private static bool InsertData(IList<string> sqlList)
        {
            ...
            try
            {
                ...
                AppleLogManager.Instance.Info(string.Format("Insert batch data to DB:{0} items", sqlList.Count));
                sqlList.Clear();
                return true;
            }
            catch (Exception ex)
