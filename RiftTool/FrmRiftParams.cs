...
namespace RiftTool
{
    public partial class FrmRift
    {
        ...
        private void LoadParmas()
        {
            ...
            gameServerList.Add(GameRiftServers.USServer);

            comboServer.DataSource = gameServerList;
            comboServer.SelectedIndex = RiftSetConfig.Instance.GameServerNo;

            if (!string.IsNullOrEmpty(RiftSetConfig.Instance.LastFile) && !RiftSetConfig.Instance.IsCompleted)
                this.txtFilePath.Text = RiftSetConfig.Instance.LastFile;
            ...
            this.comboDataType.SelectedValue = RiftSetConfig.Instance.DataFormat;

            #endregion raw data type

            #region Query Type

            QueryType qType = RiftSetConfig.Instance.QuertyType;
            ...
            if (!RiftSetConfig.Instance.IsCompleted)
            {
            ...
            this.chkReconnect.Checked = RiftSetConfig.Instance.IsSupportedReconnect;
            if (this.chkReconnect.Checked)
            {
                switch (RiftSetConfig.Instance.ReconnectType)
                {
            ...
            this.txtThreads.Text = RiftSetConfig.Instance.Threads.ToString();
            this.txtRepeatCount.Text = RiftSetConfig.Instance.ErrorRepeatCount.ToString();

            this.txtLower.Text = RiftSetConfig.Instance.RangeLower.ToString();
            this.txtUpper.Text = RiftSetConfig.Instance.RangeUpper.ToString();
