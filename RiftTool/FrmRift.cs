...
namespace RiftTool
{
    public partial class FrmRift : Form
    {
        private IList<GameServer> gameServerList = new List<GameServer>();
        private RiftHttperManager httpManager = null;
        ...
        public FrmRift()
        {
            ...
            RiftLogManager.Instance.LogEvent += LogManager_LogEvent;
        }
        ...
        private void LoadUcNet()
        {
            ...
            ucAdsl = new UcAdsl(adsl, RiftLogManager.Instance);
            ...
            ucRouter = new UcRouter(router, RiftLogManager.Instance);
            ...
            ucVpnList = new UcVpnList(vpn, RiftLogManager.Instance);
            ucVpnList.TestChanged += ucNetwork_TestChanged;

            //VPNItem vpnItem = new VPNItem();
            //vpnItem.EntryName = RiftSetConfig.Instance.VpnEntryName;
            //vpnItem.IP = RiftSetConfig.Instance.VpnIP;
            //vpnItem.User = RiftSetConfig.Instance.VpnUser;
            //vpnItem.Password = RiftSetConfig.Instance.VpnPwd;
            //ucVpn = new UcVpnItem(vpnItem, RiftLogManager.Instance);
        ...
        private void btnFile_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请选择一个（裂隙）帐户数据文件", "（裂隙）提醒");
            }
        }
        ...
        private void httpManager_ProcessItemChanged(object sender, ProcessEventArgs<RiftAccountItem> e)
        {
            lock (this)
            {
                if (this.isClosing) { return; }
                    ...
                    this.BeginInvoke(new ThreadStart(delegate()
                              {
                                  ...
                                  RiftOutputMgt.Instance.Output(e.Item, httpManager.HttperParamsItem);
                                  this.ShowProcessCount(false);
                ...
                catch (Exception ex)
                {
                    ...
                    RiftLogManager.Instance.Info("Update UI error:" + ex.Message);
                }
            }
        }
        ...
        private void ShowProcessCount(bool isRestart)
        {
            ...
            else
            {
                if (null != this.httpManager)
                {
                    ...
                    int remainCount = this.httpManager.TotalCount - RiftOutputMgt.Instance.ProcessedCount;
                    this.txtRemainCount.Text = (remainCount < 0 ? 0 : remainCount).ToString();
                }
                this.txtProcessedCount.Text = RiftOutputMgt.Instance.ProcessedCount.ToString();
                this.txtSuccessCount.Text = RiftOutputMgt.Instance.SucceedCount.ToString();
                this.txtFailedCount.Text = RiftOutputMgt.Instance.FailedCount.ToString();
                this.txtUnknowCount.Text = RiftOutputMgt.Instance.UnknownCount.ToString();
            }
        }
            ...
            this.BeginInvoke(new ThreadStart(delegate()
             {
                 ...
                 MessageBox.Show(string.Format("完成（裂隙）网络对号 ！", "（裂隙）提醒"));
                 SoundPlayer.StopWarn();
             }));
        }
        ...
        {
            this.DisposeDetectionManager();
            httpManager = new RiftHttperManager();
        ...
        private void httpManager_ProcessCompleted(object sender, BoolEventArgs e)
        {
            ...
            RiftSetConfig.Instance.IsCompleted = e.IsTrue;
        }
