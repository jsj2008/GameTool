    ...
    public class DBHelper
    {
        ...
        public static int GetHistoryCount(params LoginState[] accountStates)
        {
            lock (lockGetObj)
            {
                if (IsSqlConOpened)
                {
                    string sql = "select count(distinct Id)  as itemsCount from XBOXResult";
        ...
        public static IList<int> GetQueriedIndex()
        {
            ...
            if (IsSqlConOpened)
            {
                string sql = "select QueriedIndex from QueriedXBOX";
                using (SQLiteCommand sqlCmd = new SQLiteCommand(sql, sqlCon))
                {
                    using (SQLiteDataReader reader = sqlCmd.ExecuteReader())
                    {
        ...
        public static bool IsQueried(int userIndex)
        {
            if (IsSqlConOpened)
            {
                string sql = string.Format("SELECT QueriedIndex from QueriedXBOX where QueriedIndex={0}", userIndex);
                using (SQLiteCommand sqlCmd = new SQLiteCommand(sql, sqlCon))
                {
                    using (SQLiteDataReader reader = sqlCmd.ExecuteReader())
                    {
        ...
        public static bool ClearTable(params XBOXTableName[] tableNames)
        {
            if (IsSqlConOpened)
            {
                foreach (XBOXTableName t in tableNames)
                {
                    ClearTable(t);
                }
                return true;
        ...
        public static bool ClearTable(XBOXTableName tableName)
        {
            if (IsSqlConOpened)
            {
        ...
        public static bool InsertHistory(AccountItem account)
        {
            if (IsSqlConOpened)
            {
                string sql = string.Format(" insert into XBOXResult (AccountId,Email,State,Result)" +
                                           " values ({0},\'{1}\',\'{2}\',\'{3}\') ",
        ...
        public static bool InsertQueriedItems(AccountItem account)
        {
            lock (lockInsertObj)
            {
                if (IsSqlConOpened)
                {
                    string sql = string.Format(" insert into QueriedXBOX (QueriedIndex,Email) values ({0},\'{1}\')",
        ...
        private static bool RunSql(string sql)
        {
            if (!string.IsNullOrEmpty(sql))
            {
                ...
                {
                    XBOXLogManager.Instance.Error(string.Format("执行Insert sql时出错：{0}", ex.Message));
                }
            }
            return false;
        }
        ...
        private static bool InsertData(IList<string> sqlList)
        {
            ...
            try
            {
                ...
                XBOXLogManager.Instance.Info(string.Format("Insert batch data to DB:{0} items", sqlList.Count));
                sqlList.Clear();
                return true;
            }
            catch (Exception ex)
